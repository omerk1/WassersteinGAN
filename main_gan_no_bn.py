...
def main():
    ...
    # Model
    dsc = gan_no_bn.Discriminator(im_size).to(device)
    gen = gan_no_bn.Generator(z_dim, featuremap_size=4, out_channels=im_size[0]).to(device)
    ...
    def dsc_loss_fn(y_data, y_generated):
        return gan_no_bn.discriminator_loss_fn(y_data, y_generated, hp['data_label'], hp['label_noise'])

    def gen_loss_fn(y_generated):
        return gan_no_bn.generator_loss_fn(y_generated, hp['data_label'])
    ...
    for epoch_idx in range(num_epochs):
        ...
        with tqdm.tqdm(total=len(dl_train.batch_sampler), file=sys.stdout) as pbar:
            for batch_idx, (x_data, _) in enumerate(dl_train):
                ...
                dsc_loss, gen_loss = gan_no_bn.train_batch(
                    dsc, gen,
        ...
        plt.savefig('results/gan_no_bn/generated_samples_epoch_{}.png'.format(epoch_idx + 1))
        plt.close(fig)
    ...
    plt.savefig('results/gan_no_bn/critic_loss_per_epoch.png')
    plt.clf()
    ...
    plt.savefig('results/gan_no_bn/critic_loss_per_iteration.png')
    plt.clf()
